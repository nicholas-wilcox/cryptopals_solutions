...
module Servers
  class RSAServer < HTTPServer
    ...
    def reset_keys(bit_size = 1024)
      ...
      @n = @p * @q
      @private_key = Utils::MathUtil.invmod(3, (@p - 1) * (@q - 1))
    end
    ...
    def mount
      ...
      @server.mount_proc('/receiveMessage') { |req| @message = decrypt(req.body.hex, @private_key, @n) }
      ...
      @server.mount_proc('/sendMessage') do |req|
        ...
        post_text('/receiveMessage', JSON.parse(req.body)['port'], encrypt(@message, public_key['e'], public_key['n']).to_s(16))
      end
    end
